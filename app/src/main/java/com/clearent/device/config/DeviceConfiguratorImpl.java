...
public class DeviceConfiguratorImpl implements DeviceConfigurator {
    ...
    public DeviceConfiguratorImpl(ReaderConfigurable readerConfigurable) {
    ...
    private void setDefaultTerminalTags() {
        ...
        int setTerminalDataRt = readerConfigurable.emv_setTerminalData(tlvBytes,resDatStruct);
        if (ErrorCode.SUCCESS == setTerminalDataRt) {
            ...
            readerConfigurable.notifyCommandFailure(setTerminalDataRt, error);
        }
    }

    private void configureReader(CommunicationRequest communicationRequest) {
        GetConfigurationTaskResponseHandler getConfigurationTaskResponseHandler = new GetConfigurationTaskResponseHandler(readerConfigurable);
    ...
    private void setTerminalMajorConfiguration() {
        ...
        int commandRt = readerConfigurable.device_sendDataCommand("6016", false, "05", resDataStruct);
        if (commandRt == ErrorCode.SUCCESS) {
            ...
            readerConfigurable.notifyCommandFailure(commandRt, error);
        }
    }
    ...
    private int initClockDate() {
        ...
        ResDataStruct resDataStruct = new ResDataStruct();
        return readerConfigurable.device_sendDataCommand("2503", false, clockDate, resDataStruct);
    }
    ...
    private int initClockTime() {
        ...
        ResDataStruct resDataStruct = new ResDataStruct();
        return readerConfigurable.device_sendDataCommand("2501", false, clockTime, resDataStruct);
    }
